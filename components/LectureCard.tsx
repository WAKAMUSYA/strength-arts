...
export default function LectureCard({ lecture, baseHref, isRead = false }: Props) {
  return (
    <Link href={`${baseHref}/${lecture.id}`} className={`group block border rounded-2xl p-6 hover:shadow-md hover:-translate-y-0.5 active:scale-[0.98] active:bg-slate-100 transition-all duration-200 relative overflow-hidden ${isRead ? 'bg-slate-50 border-slate-200 opacity-90' : 'bg-white border-slate-200'}`}>
