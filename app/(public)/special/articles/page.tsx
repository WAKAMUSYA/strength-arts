...
function ArticlesContent() {
  ...
  useEffect(() => {
    ...
    if (obsParam) {
      ...
      const hasBasicMatch = SPECIAL_ARTICLES.some(art => art.type === 'basic' && art.obstacleTag === obsParam)
      const hasAppliedMatch = SPECIAL_ARTICLES.some(art => art.type === 'applied' && art.obstacleTag === obsParam)
      const hasProgramMatch = SPECIAL_ARTICLES.some(art => art.type === 'program' && art.obstacleTag === obsParam)
  ...
  const filteredArticles = SPECIAL_ARTICLES.filter(art => {
        ...
        <div className="grid grid-cols-3 gap-1 bg-zinc-950 p-1.5 rounded-2xl border border-zinc-900 shadow-inner w-full max-w-2xl">
          <button
            ...
            className={`flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-1.5 py-2 sm:py-3.5 px-1 sm:px-3 rounded-xl font-bold text-[10px] sm:text-xs transition-all duration-300 cursor-pointer ${
              activeTab === 'basic'
                ? 'bg-gradient-to-r from-indigo-950/60 to-indigo-900/40 border border-indigo-800/40 text-white shadow-md'
                : 'text-zinc-550 hover:text-zinc-300'
            }`}
          >
            <BookOpen className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${activeTab === 'basic' ? 'text-indigo-400' : 'text-zinc-600'}`} />
            <span>特別コラム</span>
          </button>
          <button
          ...
          >
            <Layers className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${activeTab === 'applied' ? 'text-indigo-400' : 'text-zinc-600'}`} />
            <span>武術・バイオメカニクス</span>
          </button>
          <button
          ...
          >
            <Activity className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${activeTab === 'program' ? 'text-indigo-400' : 'text-zinc-600'}`} />
            <span>実践行法</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {filteredArticles.map((art) => (
          <Link
            ...
            className="group cursor-pointer bg-zinc-950/40 border border-zinc-900 hover:border-indigo-900/60 hover:bg-zinc-900/10 rounded-2xl p-6 transition-all duration-500 flex flex-col justify-between hover:shadow-2xl hover:shadow-indigo-950/10 relative overflow-hidden"
          >
            {/* Soft decorative accent glow */}
            <div className="absolute top-0 right-0 w-24 h-24 bg-indigo-500/5 rounded-full blur-2xl group-hover:bg-indigo-500/10 transition-colors duration-500" />
            ...
            <div className="space-y-5 relative z-10">
              ...
              <div className="flex items-center justify-between text-[10px] font-mono text-zinc-500 border-b border-zinc-900/60 pb-3">
                <span className="uppercase tracking-widest text-indigo-500 font-extrabold flex items-center gap-1">
                  <Bookmark className="w-3.5 h-3.5 text-indigo-500" /> {art.category}
                </span>
              ...
              <div className="space-y-1">
                <h3 className="text-lg md:text-xl font-black text-white group-hover:text-indigo-400 transition-colors leading-tight">
                  {art.title}
                </h3>
              </div>
            ...
            <div className="mt-8 pt-4 border-t border-zinc-900 flex flex-wrap items-center justify-between gap-3 text-[11px] font-mono text-zinc-500 relative z-10">
              ...
              <span className="text-zinc-400 group-hover:text-indigo-400 group-hover:translate-x-1 transition-all flex items-center gap-1 font-bold">
                解説を開く <ChevronRight className="w-4 h-4 text-indigo-500" />
              </span>
            </div>
          </Link>
        ))}
    ...
    <main className="min-h-screen bg-black text-white selection:bg-indigo-900 selection:text-white pb-32">
      ...
      <nav className="border-b border-zinc-900 bg-zinc-950/40 backdrop-blur-md sticky top-16 z-30 px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <Link 
            href="/special" 
            className="text-xs font-mono font-bold text-zinc-500 hover:text-indigo-500 transition-colors inline-flex items-center gap-1 cursor-pointer"
          >
            <ArrowLeft className="w-4 h-4" /> BACK TO LAB
          </Link>
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1 text-[8px] font-extrabold text-indigo-500 bg-indigo-950/40 border border-indigo-900/40 px-2.5 py-0.5 rounded-full uppercase tracking-wider">
              <Compass className="w-3.5 h-3.5 text-indigo-500" /> LECTURE INDEX
            </span>
            <span className="text-[10px] text-zinc-550 font-mono hidden sm:inline">SA-SP-LECTURES</span>
          </div>
        </div>
      </nav>
      ...
      <header className="max-w-6xl mx-auto px-6 pt-16 pb-8 text-center space-y-4">
        <h1 className="text-3xl md:text-5xl font-black tracking-tight text-white">
          武道・哲学研究所<br className="sm:hidden" /> コラムアーカイブ
        </h1>
      </header>
