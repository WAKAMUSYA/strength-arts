...
function ArticlesContent() {
  ...
  useEffect(() => {
    ...
    if (obsParam) {
      ...
      const hasBasicMatch = DEADLIFT_ARTICLES.some(art => art.type === 'basic' && art.obstacleTag === obsParam)
      const hasAppliedMatch = DEADLIFT_ARTICLES.some(art => art.type === 'applied' && art.obstacleTag === obsParam)
      const hasProgramMatch = DEADLIFT_ARTICLES.some(art => art.type === 'program' && art.obstacleTag === obsParam)
  ...
  const filteredArticles = DEADLIFT_ARTICLES.filter(art => {
        ...
        <div className="grid grid-cols-3 gap-1 bg-zinc-950 p-1.5 rounded-2xl border border-zinc-900 shadow-inner w-full max-w-2xl">
          <button
            ...
            className={`flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-1.5 py-2 sm:py-3.5 px-1 sm:px-3 rounded-xl font-bold text-[10px] sm:text-xs transition-all duration-300 cursor-pointer ${
              activeTab === 'basic'
                ? 'bg-gradient-to-r from-red-950/60 to-red-900/40 border border-emerald-800/40 text-white shadow-md'
                : 'text-zinc-550 hover:text-zinc-300'
            }`}
          >
            <BookOpen className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${activeTab === 'basic' ? 'text-emerald-400' : 'text-zinc-600'}`} />
            <span>基本理論</span>
          </button>
          <button
          ...
          >
            <Layers className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${activeTab === 'applied' ? 'text-emerald-400' : 'text-zinc-600'}`} />
            <span>応用・探究</span>
          </button>
          <button
          ...
          >
            <Activity className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${activeTab === 'program' ? 'text-emerald-400' : 'text-zinc-600'}`} />
            <span>実践プログラム</span>
          </button>
        </div>
      </div>
          ...
          >
            ...
            <div className="space-y-5 relative z-10">
              ...
              <div className="flex items-center justify-between text-[10px] font-mono text-zinc-500 border-b border-zinc-900/60 pb-3">
                <span className="uppercase tracking-widest text-emerald-500 font-extrabold flex items-center gap-1">
                  <Bookmark className="w-3.5 h-3.5 text-emerald-500" /> {art.category}
                </span>
...
export default function DeadliftArticlesPage() {
  return (
    <main className="min-h-screen bg-black text-white selection:bg-emerald-900 selection:text-white pb-32">
      ...
      <nav className="border-b border-zinc-900 bg-zinc-950/40 backdrop-blur-md sticky top-16 z-30 px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-zinc-550 font-mono hidden sm:inline">SA-DL-LECTURES</span>
          </div>
        </div>
      </nav>
      ...
      <header className="max-w-6xl mx-auto px-6 pt-16 pb-8 text-center space-y-4">
        <h1 className="text-3xl md:text-5xl font-black tracking-tight text-white">
          デッドリフト研究所<br className="sm:hidden" /> コラムアーカイブ
        </h1>
      </header>
