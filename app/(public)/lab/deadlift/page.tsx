...
const OBSTACLES = [
  { label: '重量が伸びない', desc: '床から全く浮かない、または膝下で止まる' },
  { label: '腰が痛い', desc: '引いた翌日に腰痛がひどい' },
  { label: '背中が曲がる', desc: 'ラウンドバックしてしまう' },
  { label: 'グリップが持たない', desc: '手が滑ってバーを落としてしまう' }
]

const RESEARCH_PAPERS: ResearchPaper[] = [
  {
    id: 'res-dl-1',
    title: 'コンベンショナルとスモウ・デッドリフトのキネマティクスとキネティクスの比較',
    journal: 'Medicine and Science in Sports and Exercise',
    year: 2000,
    doi: '10.1097/00005768-200007000-00013',
    findings: 'コンベンショナルはL4/L5脊椎セグメントにかかるせん断力がスモウよりも有意に大きく、スモウは大腿四頭筋と前脛骨筋の活動が有意に大きいことが判明した。',
    citationCount: 412,
    tag: 'スタンス比較'
  },
  {
    id: 'res-dl-2',
    title: 'デッドリフトにおけるグリップ幅が筋活動と挙上距離に及ぼす影響',
    journal: 'Journal of Strength and Conditioning Research',
    year: 2011,
    doi: '10.1519/JSC.0b013e3181f710b8',
    findings: 'グリップ幅を狭くすると挙上距離が短縮され、バイオメカニクス的仕事量が減少するが、広背筋への要求は維持される。',
    citationCount: 154,
    tag: 'グリップ幅'
  },
  {
    id: 'res-dl-3',
    title: 'デッドリフト時の脊柱起立筋とハムストリングスの同時活動（Co-activation）',
    journal: 'Journal of Biomechanics',
    year: 2018,
    doi: '10.1016/j.jbiomech.2017.11.004',
    findings: 'ヒップヒンジの最下点において、脊柱起立筋のアイソメトリック収縮とハムストリングスの伸張性収縮の強い共収縮が、骨盤の安定性に不可欠である。',
    citationCount: 221,
    tag: 'ヒップヒンジ'
  }
]
...
export default function DeadliftLab() {

  const mustReadArticles = useMemo(() => {
    return DEADLIFT_ARTICLES.filter(art => art.type === 'basic')
  }, [])

  const columnArticles = useMemo(() => {
    return DEADLIFT_ARTICLES.filter(art => art.type === 'applied')
  }, [])

  const roadmapSteps = useMemo(() => {
    return DEADLIFT_ARTICLES
      .filter(art => typeof art.roadmapNumber === 'number')
      .sort((a, b) => (a.roadmapNumber || 0) - (b.roadmapNumber || 0))
  }, [])

  const programArticles = useMemo(() => {
    return DEADLIFT_ARTICLES.filter(art => art.type === 'program')
  }, [])
  ...
  return (
    <main className="min-h-screen bg-black text-white selection:bg-emerald-900 selection:text-white pb-32">
      ...
      <section className="relative overflow-hidden border-b border-zinc-900 pt-32 pb-20 bg-zinc-950">
        <div className="absolute inset-0 bg-[url('/dedlift.jpg')] bg-cover bg-center bg-no-repeat opacity-60 grayscale-[50%]" />
        ...
        <div className="max-w-3xl mx-auto px-4 relative z-10">
          <div className="bg-black/60 backdrop-blur-md rounded-3xl border border-white/10 shadow-2xl p-6 md:p-10 text-center space-y-6">
            <div className="flex items-center justify-center gap-2">
              <span className="inline-flex items-center gap-1.5 text-[9px] font-extrabold text-emerald-500 bg-emerald-950/40 border border-emerald-900/40 px-3 py-1 rounded-full uppercase tracking-widest">
                <Activity className="w-3.5 h-3.5 text-emerald-500" /> Deadlift Mechanics
              </span>
              <span className="text-[10px] text-zinc-400 font-mono">CODE: SQ-LAB-01</span>
            </div>

            <h1 className="text-4xl md:text-6xl font-black tracking-tight text-white leading-tight">
              デッドリフト<span className="text-emerald-500 font-extrabold relative inline-block">研究所</span>
            </h1>
            ...
            <div className="pt-6 flex justify-center">
              <Link
                ...
                className="group relative inline-flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-black font-bold py-3.5 px-8 rounded-full transition-all duration-300 hover:shadow-lg hover:shadow-emerald-900/40 border border-emerald-500"
              >
              ...
              >
                <div className="space-y-4">
                  <div className="relative aspect-[16/10] w-full rounded-lg overflow-hidden bg-zinc-900">
                    ...
                    />
                    <span className="absolute top-2 left-2 z-20 text-[8px] font-extrabold text-emerald-500 bg-zinc-950/90 border border-zinc-900 px-2 py-0.5 rounded tracking-widest uppercase">
                      LECTURE 0{idx + 1}
                    </span>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-[9px] font-mono text-zinc-500">
                      <span className="uppercase tracking-widest text-emerald-500 font-semibold">{art.category}</span>
                      <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> {art.readTime}</span>
                    </div>
                    <h3 className="text-sm md:text-base font-bold text-white group-hover:text-emerald-500 transition-colors leading-snug">
                      {art.title}
                    </h3>
                ...
                <div className="mt-6 pt-3 border-t border-zinc-900 flex items-center justify-between text-[10px] font-mono text-zinc-500">
                  ...
                  <span className="text-zinc-400 group-hover:text-emerald-500 group-hover:translate-x-1 transition-all flex items-center gap-1 font-bold">
      ...
      <section className="py-20 bg-zinc-950/20 border-b border-zinc-900">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center space-y-3 mb-16">
            <span className="text-[10px] font-extrabold text-emerald-500 tracking-wider uppercase bg-emerald-950/40 border border-emerald-900/40 px-3 py-1 rounded-full">
              STRUCTURED PATHWAY
            </span>
            <h2 className="text-2xl md:text-3xl font-black text-white tracking-tight">
              順番に学ぶ
              ...
              {roadmapSteps.map((st, idx) => (
                ...
                >
                  {idx < roadmapSteps.length - 1 && (
                    <div className="hidden lg:block absolute -right-3 top-1/2 -translate-y-1/2 z-20 bg-zinc-950 rounded-full p-1 border border-zinc-900 text-zinc-650 group-hover:text-emerald-500 group-hover:bg-emerald-950/30 transition-colors">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="text-2xl font-black text-emerald-600/80 group-hover:text-emerald-500 transition-colors font-mono">
                        0{st.roadmapNumber}
                      </span>
                    ...
                    <div className="space-y-1.5">
                      <p className="text-[9px] font-mono text-emerald-500/80 uppercase tracking-wide">
                        {st.category}
                      </p>
                      <h3 className="text-sm font-bold text-white group-hover:text-emerald-500 transition-colors leading-tight">
                        {st.title}
                      </h3>
                  ...
                  <div className="mt-8 pt-3 border-t border-zinc-900 flex items-center justify-between text-[9px] font-mono text-zinc-500">
                    <span>DL-STEP-{st.slug.slice(0, 6)}</span>
                    <span className="text-zinc-400 group-hover:text-emerald-500 font-bold flex items-center gap-0.5">
                ...
                <div className="space-y-4">
                  <div className="relative aspect-[16/10] w-full rounded-lg overflow-hidden bg-zinc-900">
                    ...
                    <img
                      ...
                      className="object-cover w-full h-full group-hover:scale-105 transition-transform duration-500 opacity-80"
                    />
                    <span className="absolute top-2 left-2 z-20 text-[8px] font-extrabold text-white bg-emerald-600/90 border border-emerald-500 px-2 py-0.5 rounded tracking-widest uppercase">
                      PROGRAM
                    </span>
                  </div>
      ...
      <section className="py-20 bg-black border-b border-zinc-900">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-4">
            {OBSTACLES.map((obs) => (
              <Link
                href={`/lab/deadlift/articles?obstacle=${obs.label}`}
                key={obs.label}
              ...
              >
                <div className="space-y-2">
                  <span className="text-[8px] font-extrabold font-mono tracking-widest uppercase block text-zinc-550 group-hover:text-emerald-500">
                    OBSTACLE
                  </span>
              ...
              >
                <div className="space-y-4">
                  <div className="flex items-start justify-between border-b border-zinc-900 pb-3">
                    ...
                    <span className="text-[8px] font-bold text-emerald-500 bg-emerald-950/50 border border-emerald-900/50 px-2.5 py-0.5 rounded-full uppercase tracking-wider font-mono">
                      {paper.tag}
                    </span>
                  </div>

                  <h3 className="text-sm md:text-base font-extrabold text-white leading-snug group-hover:text-emerald-500 transition-colors">
                    {paper.title}
                  </h3>
                ...
                <div className="mt-6 pt-4 border-t border-zinc-900 flex items-center justify-between text-[9px] font-mono text-zinc-500">
                  ...
                  <a
                    ...
                    className="flex items-center gap-1 text-zinc-450 hover:text-emerald-500 transition-colors font-bold"
                  >
