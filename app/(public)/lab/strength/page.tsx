...
const RESEARCH_PAPERS: ResearchPaper[] = [
  {
    id: 'res-str-1',
    title: 'レジスタンストレーニング初期における神経系適応の寄与',
    journal: 'Journal of Applied Physiology',
    year: 1988,
    doi: '10.1152/jappl.1988.64.4.1590',
    findings: 'トレーニング開始から最初の数週間において、筋力の向上は主に神経系の適応（運動単位の動員増加と発火頻度の向上）によるものであり、筋肥大の寄与は小さい。',
    citationCount: 1250,
    tag: '神経適応'
  },
  {
    id: 'res-str-2',
    title: '線形および非線形ピリオダイゼーションが最大筋力に及ぼす影響',
    journal: 'Journal of Strength and Conditioning Research',
    year: 2002,
    doi: '10.1519/1533-4287(2002)016<0250:ACOMUA>2.0.CO;2',
    findings: '非線形（波状）ピリオダイゼーションは、従来の線形ピリオダイゼーションと比較して、ベンチプレスおよびスクワットの1RMを有意に向上させる。',
    citationCount: 485,
    tag: 'プログラム'
  },
  {
    id: 'res-str-3',
    title: '異なる強度のトレーニングが筋肥大および最大筋力に与える影響',
    journal: 'Medicine and Science in Sports and Exercise',
    year: 2015,
    doi: '10.1249/MSS.0000000000000672',
    findings: '高強度・低回数のトレーニングは最大筋力（1RM）の向上において低強度・高回数を上回るが、筋肥大の程度には両グループ間で有意な差は見られなかった。',
    citationCount: 890,
    tag: 'トレーニング強度'
  }
]
...
const RELATED_LABS = [
  { title: '筋肥大研究所', label: 'Hypertrophy Hub', slug: 'hypertrophy', glowColor: 'hover:border-red-500/40 hover:bg-red-950/10' },
...
export default function StrengthLab() {
  ...
  const mustReadArticles = useMemo(() => {
    return STRENGTH_ARTICLES.filter(art => art.type === 'basic')
  }, [])

  const columnArticles = useMemo(() => {
    return STRENGTH_ARTICLES.filter(art => art.type === 'applied')
  }, [])

  const roadmapSteps = useMemo(() => {
    return STRENGTH_ARTICLES
      .filter(art => typeof art.roadmapNumber === 'number')
      .sort((a, b) => (a.roadmapNumber || 0) - (b.roadmapNumber || 0))
  }, [])

  const programArticles = useMemo(() => {
    return STRENGTH_ARTICLES.filter(art => art.type === 'program')
  }, [])
...
{/* ----------------- SECTION ①: HERO ----------------- */}
      <section className="relative overflow-hidden border-b border-zinc-900 pt-32 pb-20 bg-zinc-950">
        <div className="absolute inset-0 bg-[url('/strength.jpg')] bg-cover bg-center bg-no-repeat opacity-60 grayscale-[50%]" />
          ...
          <div className="bg-black/60 backdrop-blur-md rounded-3xl border border-white/10 shadow-2xl p-6 md:p-10 text-center space-y-6">
            <div className="flex items-center justify-center gap-2">
              <span className="inline-flex items-center gap-1.5 text-[9px] font-extrabold text-red-500 bg-red-950/40 border border-red-900/40 px-3 py-1 rounded-full uppercase tracking-widest">
                <Activity className="w-3.5 h-3.5 text-red-500" /> Strength Mechanics
              </span>
              <span className="text-[10px] text-zinc-400 font-mono">CODE: STR-LAB-01</span>
            </div>

            <h1 className="text-4xl md:text-6xl font-black tracking-tight text-white leading-tight">
              筋力<span className="text-red-500 font-extrabold relative inline-block">研究所</span>
            </h1>
              ...
              {roadmapSteps.map((st, idx) => (
                ...
                >
                  ...
                  <div className="mt-8 pt-3 border-t border-zinc-900 flex items-center justify-between text-[9px] font-mono text-zinc-500">
                    <span>STR-STEP-{st.slug.slice(0, 6)}</span>
      ...
      <section className="py-20 bg-black border-b border-zinc-900">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-4">
            {OBSTACLES.map((obs) => (
              <Link
                href={`/lab/strength/articles?obstacle=${obs.label}`}
                key={obs.label}
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-lg bg-black border border-zinc-900 text-xs md:text-sm text-zinc-450 leading-relaxed font-light shadow-inner">
                    <span className="font-bold text-zinc-300 block mb-1 text-xs">【生理学的知見・エビデンス】</span>
                    {paper.findings}
                  </div>
                </div>
