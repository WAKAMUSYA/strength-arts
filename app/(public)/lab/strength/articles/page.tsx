...
const OBSTACLES = [
  ...
  { label: '神経疲労', desc: 'トレーニングの質が落ちている' }
]
...
function ArticlesContent() {
  ...
  useEffect(() => {
    ...
    if (obsParam) {
      ...
      const hasBasicMatch = STRENGTH_ARTICLES.some(art => art.type === 'basic' && art.obstacleTag === obsParam)
      const hasAppliedMatch = STRENGTH_ARTICLES.some(art => art.type === 'applied' && art.obstacleTag === obsParam)
      const hasProgramMatch = STRENGTH_ARTICLES.some(art => art.type === 'program' && art.obstacleTag === obsParam)
  ...
  const filteredArticles = STRENGTH_ARTICLES.filter(art => {
...
export default function StrengthArticlesPage() {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
    ...
    <main className="min-h-screen bg-black text-white selection:bg-red-900 selection:text-white pb-32">
      ...
      <nav className="border-b border-zinc-900 bg-zinc-950/40 backdrop-blur-md sticky top-16 z-30 px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-zinc-550 font-mono hidden sm:inline">SA-STR-LECTURES</span>
          </div>
        </div>
      </nav>
      ...
      <header className="max-w-6xl mx-auto px-6 pt-16 pb-8 text-center space-y-4">
        <h1 className="text-3xl md:text-5xl font-black tracking-tight text-white">
          筋力研究所<br className="sm:hidden" /> コラムアーカイブ
        </h1>
      </header>
