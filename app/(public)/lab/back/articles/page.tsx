...
function ArticlesContent() {
  ...
  const filteredArticles = BACK_ARTICLES.filter(art => {
          ...
          >
            ...
            <div className="space-y-5 relative z-10">
              ...
              <div className="flex items-center justify-between text-[10px] font-mono text-zinc-500 border-b border-zinc-900/60 pb-3">
                <span className="uppercase tracking-widest text-emerald-450 font-extrabold flex items-center gap-1">
    ...
    <main className="min-h-screen bg-black text-white selection:bg-emerald-900 selection:text-white pb-32">
      ...
      <nav className="border-b border-zinc-900 bg-zinc-950/40 backdrop-blur-md sticky top-16 z-30 px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <Link 
            href="/lab/back" 
            className="text-xs font-mono font-bold text-zinc-500 hover:text-emerald-450 transition-colors inline-flex items-center gap-1 cursor-pointer"
          >
            <ArrowLeft className="w-4 h-4" /> BACK TO LAB
          </Link>
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1 text-[8px] font-extrabold text-emerald-400 bg-emerald-950/40 border border-emerald-900/40 px-2.5 py-0.5 rounded-full uppercase tracking-wider">
              <Compass className="w-3.5 h-3.5 text-emerald-500" /> LECTURE INDEX
            </span>
            <span className="text-[10px] text-zinc-550 font-mono hidden sm:inline">SA-BK-LECTURES</span>
          </div>
        </div>
      </nav>
      ...
      <header className="max-w-6xl mx-auto px-6 pt-16 pb-8 text-center space-y-4">
        <h1 className="text-3xl md:text-5xl font-black tracking-tight text-white">
          背中トレ研究所<br className="sm:hidden" /> コラムアーカイブ
        </h1>
      </header>
