...
export default function ArticleDetailPage() {
  ...
  const articleIndex = POWER_ARTICLES.findIndex(art => art.slug === slug)
  const article = POWER_ARTICLES[articleIndex]
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-6 text-center text-zinc-100">
        ...
        <Link 
          ...
          className="text-xs font-mono font-bold text-orange-400 hover:underline inline-flex items-center gap-1"
        >
  ...
  const nextArticle = POWER_ARTICLES[articleIndex + 1] || null
  ...
  const getNoteIcon = (type: 'warning' | 'info' | 'success') => {
    switch (type) {
      ...
      case 'info':
      default:
        return <Info className="w-5 h-5 shrink-0 mt-0.5 text-orange-400" />
    }
  }

  return (
    <main className="min-h-screen bg-zinc-950 text-zinc-200 font-sans antialiased selection:bg-orange-900/50 selection:text-orange-100 pb-32">
      ...
      <div className="sticky top-16 z-30 bg-zinc-950/80 backdrop-blur-md border-b border-zinc-900 px-6 py-3.5 flex items-center justify-between text-xs text-zinc-500 font-mono">
        ...
        <div className="hidden sm:flex items-center gap-4 shrink-0">
          <span className="flex items-center gap-1 uppercase text-orange-400 font-bold"><Activity className="w-3.5 h-3.5" /> {article.category}</span>
      ...
      <div className="max-w-4xl mx-auto px-6 mt-12">
        ...
        <div className="space-y-4 mb-10 pb-8 border-b border-zinc-900">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1 text-[9px] font-extrabold text-orange-400 bg-orange-950/50 border border-orange-900/50 px-2.5 py-0.5 rounded-full uppercase tracking-wider">
        ...
        <div className="p-6 rounded-2xl bg-zinc-900/40 border border-zinc-900 shadow-sm mb-12 space-y-3">
          <h3 className="text-xs font-mono font-extrabold text-orange-400 flex items-center gap-1.5 uppercase tracking-widest">
            <Compass className="w-4 h-4 text-orange-400" /> 要約
          </h3>
          ...
          {article.sections.map((sect, i) => (
            <div key={i} className="space-y-6">
              ...
              <h2 className="text-xl md:text-2xl font-black text-white tracking-tight border-b border-zinc-900 pb-3 flex items-center gap-2">
                <CornerDownRight className="w-5 h-5 text-orange-400 shrink-0" />
                {sect.title}
              </h2>
                ...
                <ul className="space-y-3.5 pl-1.5 my-6">
                  {sect.bullets.map((bullet, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      <span className="w-5 h-5 rounded-full bg-orange-950/50 border border-orange-900/40 text-orange-400 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">
                        ✓
                      </span>
        ...
        <ArticleInteractionsClient articleId={`power/${slug}`} />
