...
export default async function SquatArticleDetail({ params }: { params: { slug: string } }) {
  const { slug } = params
  
  const articleIndex = SQUAT_ARTICLES.findIndex(art => art.slug === slug)
  const article = SQUAT_ARTICLES[articleIndex]
  ...
  const prevArticle = articleIndex > 0 ? SQUAT_ARTICLES[articleIndex - 1] : null
  const nextArticleInList = articleIndex < SQUAT_ARTICLES.length - 1 ? SQUAT_ARTICLES[articleIndex + 1] : null
    ...
    <main className="min-h-screen bg-black text-white selection:bg-red-900 selection:text-white pb-32">
      ...
      <article className="max-w-3xl mx-auto px-6 py-12 relative z-30">
        ...
        <div className="mt-12 flex flex-wrap gap-2">
          {article.tags && article.tags.map(tag => (
            <Link 
              href={`/lab/squat/articles?tag=${tag}`} 
              key={tag}
      ...
      <div className="max-w-4xl mx-auto px-6 mt-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {prevArticle ? (
            <Link 
              href={`/lab/squat/${prevArticle.slug}`}
          ...
          {(article.nextArticle || nextArticleInList) ? (
            <Link 
              href={`/lab/squat/${article.nextArticle?.slug || nextArticleInList?.slug}`}
