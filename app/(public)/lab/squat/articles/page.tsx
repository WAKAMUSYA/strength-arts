...
function ArticlesContent() {
  ...
  useEffect(() => {
    const fetchStatuses = async () => {
      const ids = SQUAT_ARTICLES.map(art => `squat/${art.slug}`)
      try {
  ...
  useEffect(() => {
    ...
    if (obsParam) {
      ...
      const hasBasicMatch = SQUAT_ARTICLES.some(art => art.type === 'basic' && art.obstacleTag === obsParam)
      const hasAppliedMatch = SQUAT_ARTICLES.some(art => art.type === 'applied' && art.obstacleTag === obsParam)
      const hasProgramMatch = SQUAT_ARTICLES.some(art => art.type === 'program' && art.obstacleTag === obsParam)
  ...
  const filteredArticles = SQUAT_ARTICLES.filter(art => {
    ...
    <div className="max-w-6xl mx-auto px-6 mt-8">
      ...
      <div className="flex justify-center mb-12">
        <div className="grid grid-cols-3 gap-1 bg-zinc-950 p-1.5 rounded-2xl border border-zinc-900 shadow-inner w-full max-w-2xl">
          ...
          >
            <BookOpen className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${activeTab === 'basic' ? 'text-red-400' : 'text-zinc-600'}`} />
            <span>基本理論</span>
          </button>
          <button
          ...
          >
            <Layers className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${activeTab === 'applied' ? 'text-red-400' : 'text-zinc-600'}`} />
            <span>応用・探究</span>
          </button>
          <button
          ...
          >
            <Activity className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${activeTab === 'program' ? 'text-red-400' : 'text-zinc-600'}`} />
            <span>実践プログラム</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        {filteredArticles.map((art) => {
          const articleId = `squat/${art.slug}`;
...
export default function SquatArticlesPage() {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
    ...
    <main className="min-h-screen bg-black text-white selection:bg-red-900 selection:text-white pb-32">
      ...
      <nav className="border-b border-zinc-900 bg-zinc-950/40 backdrop-blur-md sticky top-16 z-30 px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <Link 
            href="/lab/squat" 
            className="text-xs font-mono font-bold text-zinc-500 hover:text-red-500 transition-colors inline-flex items-center gap-1 cursor-pointer"
          >
            <ArrowLeft className="w-4 h-4" /> BACK TO LAB
          </Link>
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1 text-[8px] font-extrabold text-red-500 bg-red-950/40 border border-red-900/40 px-2.5 py-0.5 rounded-full uppercase tracking-wider">
              <Compass className="w-3.5 h-3.5 text-red-500" /> LECTURE INDEX
            </span>
            <span className="text-[10px] text-zinc-550 font-mono hidden sm:inline">SA-SQ-LECTURES</span>
          </div>
        </div>
      </nav>
      ...
      <header className="max-w-6xl mx-auto px-6 pt-16 pb-8 text-center space-y-4">
        <h1 className="text-3xl md:text-5xl font-black tracking-tight text-white">
          スクワット研究所<br className="sm:hidden" /> コラムアーカイブ
        </h1>
      </header>
