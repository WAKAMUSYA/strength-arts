...
export default function ShoulderArticleDetail() {
  ...
  const articleIndex = SHOULDER_ARTICLES.findIndex(art => art.slug === slug)
  const article = SHOULDER_ARTICLES[articleIndex]
  ...
  const prevArticle = articleIndex > 0 ? SHOULDER_ARTICLES[articleIndex - 1] : null
  const nextArticleInList = articleIndex < SHOULDER_ARTICLES.length - 1 ? SHOULDER_ARTICLES[articleIndex + 1] : null
    ...
    <main className="min-h-screen bg-black text-white selection:bg-purple-900 selection:text-white pb-32">
      ...
      <nav className="border-b border-zinc-900 bg-zinc-950/80 backdrop-blur-md sticky top-16 z-40 px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <button 
            onClick={() => router.back()}
            className="text-xs font-mono font-bold text-zinc-400 hover:text-purple-400 transition-colors inline-flex items-center gap-1.5 cursor-pointer"
          >
        ...
        <div className="relative z-30 max-w-4xl mx-auto px-6 pt-32 pb-16">
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs font-mono">
              <span className="bg-purple-950/80 border border-purple-900 text-purple-400 px-3 py-1 rounded-full font-bold tracking-widest flex items-center gap-1.5">
            ...
            <div className="space-y-4 max-w-3xl">
              ...
              <p className="text-lg md:text-xl text-purple-400/90 font-bold leading-relaxed">
                {article.subtitle}
              </p>
            </div>
      ...
      <article className="max-w-3xl mx-auto px-6 py-12 relative z-30">
        ...
        {/* Overview Box */}
        <div className="bg-zinc-950/50 border-l-4 border-purple-500 p-6 md:p-8 rounded-r-2xl mb-16 text-zinc-300 leading-relaxed font-light text-[15px] md:text-[17px]">
          ...
          {article.sections.map((section, idx) => (
            <section key={idx} className="space-y-6">
              ...
              <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight flex items-center gap-3">
                <span className="text-purple-500/70 text-4xl font-black font-mono">{(idx + 1).toString().padStart(2, '0')}</span>
                {section.title}
              </h2>
              ...
              {section.note && (
                <div className={`mt-8 p-5 rounded-xl border flex gap-4 items-start ${
                  ...
                  section.note.type === 'success' ? 'bg-purple-950/20 border-purple-900/50 text-purple-200' :
                  ...
                  <div className="shrink-0 mt-0.5">
                    ...
                    {section.note.type === 'success' && <CheckCircle2 className="w-5 h-5 text-purple-500" />}
        ...
        <div className="mt-12 flex flex-wrap gap-2">
          {article.tags.map(tag => (
            <Link 
              href={`/lab/shoulder/articles?tag=${tag}`} 
              key={tag}
      ...
      <div className="max-w-4xl mx-auto px-6 mt-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {prevArticle ? (
            <Link 
              href={`/lab/shoulder/${prevArticle.slug}`}
            ...
            >
              ...
              <h4 className="font-bold text-white text-sm md:text-base leading-tight group-hover:text-purple-400 transition-colors">
                {prevArticle.title}
              </h4>
            </Link>
          ) : <div />}
          ...
          {(article.nextArticle || nextArticleInList) ? (
            <Link 
              href={`/lab/shoulder/${article.nextArticle?.slug || nextArticleInList?.slug}`}
              className="group p-6 rounded-2xl border border-zinc-900 bg-zinc-950 hover:border-purple-900/50 hover:bg-purple-950/10 transition-all flex flex-col justify-center text-right"
            >
              <div className="flex items-center justify-end gap-2 text-zinc-500 text-xs font-mono mb-2 group-hover:text-purple-400 transition-colors">
