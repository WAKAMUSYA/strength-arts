...
export default function ArmArticleDetail() {
  ...
  const articleIndex = ARM_ARTICLES.findIndex(art => art.slug === slug)
  const article = ARM_ARTICLES[articleIndex]
  ...
  const prevArticle = articleIndex > 0 ? ARM_ARTICLES[articleIndex - 1] : null
  const nextArticleInList = articleIndex < ARM_ARTICLES.length - 1 ? ARM_ARTICLES[articleIndex + 1] : null
    ...
    <main className="min-h-screen bg-black text-white selection:bg-red-900 selection:text-white pb-32">
      ...
      <nav className="border-b border-zinc-900 bg-zinc-950/80 backdrop-blur-md sticky top-16 z-40 px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <button 
            onClick={() => router.back()}
            className="text-xs font-mono font-bold text-zinc-400 hover:text-red-400 transition-colors inline-flex items-center gap-1.5 cursor-pointer"
          >
        ...
        <div className="relative z-30 max-w-4xl mx-auto px-6 pt-32 pb-16">
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs font-mono">
              <span className="bg-red-950/80 border border-red-900 text-red-400 px-3 py-1 rounded-full font-bold tracking-widest flex items-center gap-1.5">
            ...
            <div className="space-y-4 max-w-3xl">
              ...
              <p className="text-lg md:text-xl text-red-400/90 font-bold leading-relaxed">
                {article.subtitle}
              </p>
            </div>
      ...
      <article className="max-w-3xl mx-auto px-6 py-12 relative z-30">
        ...
        <div className="mt-12 flex flex-wrap gap-2">
          {article.tags.map(tag => (
            <Link 
              href={`/lab/arms/articles?tag=${tag}`} 
              key={tag}
      ...
      <div className="max-w-4xl mx-auto px-6 mt-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {prevArticle ? (
            <Link 
              href={`/lab/arms/${prevArticle.slug}`}
          ...
          {(article.nextArticle || nextArticleInList) ? (
            <Link 
              href={`/lab/arms/${article.nextArticle?.slug || nextArticleInList?.slug}`}
