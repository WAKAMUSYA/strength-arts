...
const RESEARCH_PAPERS: ResearchPaper[] = [
  {
    id: 'res-golf-1',
    title: 'ゴルフスイングにおけるキネマティック・シーケンスとクラブヘッド速度の相関',
    journal: 'Journal of Sports Sciences',
    year: 2008,
    doi: '10.1080/02640410801930194',
    findings: 'プロゴルファーはアマチュアに比べ、骨盤→胸郭→腕→クラブの順序での加速と減速（シーケンス）が極めて明確であり、これがヘッドスピードの決定的な要因であった。',
    citationCount: 412,
    tag: '運動連鎖'
  },
  {
    id: 'res-golf-2',
    title: 'ダウンスイング時の垂直方向の床反力（GRF）と飛距離の関係',
    journal: 'Sports Biomechanics',
    year: 2016,
    doi: '10.1080/14763141.2016.1142491',
    findings: 'PGAツアー選手のデータ解析により、左足への強烈な垂直方向の床反力（体重の約2倍）が、骨盤の回転速度およびクラブヘッド速度の増大と強く相関することが判明した。',
    citationCount: 325,
    tag: '床反力/GRF'
  },
  {
    id: 'res-golf-3',
    title: 'ゴルフスイングにおけるアーリーエクステンションの生体力学的要因と腰椎への負荷',
    journal: 'International Journal of Sports Physical Therapy',
    year: 2020,
    doi: '10.26603/ijspt20200588',
    findings: '股関節（特にリードレッグ）の内旋可動域の制限がアーリーエクステンションの主原因であり、これは腰椎への過剰なストレス（腰痛）に直結することが示唆された。',
    citationCount: 154,
    tag: '障害予防'
  }
]
...
export default function GolfLab() {

  const mustReadArticles = useMemo(() => {
    return GOLF_ARTICLES.filter(art => art.type === 'basic')
  }, [])

  const columnArticles = useMemo(() => {
    return GOLF_ARTICLES.filter(art => art.type === 'applied')
  }, [])

  const roadmapSteps = useMemo(() => {
    return GOLF_ARTICLES
      .filter(art => typeof art.roadmapNumber === 'number')
      .sort((a, b) => (a.roadmapNumber || 0) - (b.roadmapNumber || 0))
  }, [])

  const programArticles = useMemo(() => {
    return GOLF_ARTICLES.filter(art => art.type === 'program')
  }, [])
...
{/* ----------------- SECTION ①: HERO ----------------- */}
      <section className="relative overflow-hidden border-b border-zinc-900 pt-32 pb-20 bg-zinc-950">
        <div className="absolute inset-0 bg-[url('/golf.jpg')] bg-cover bg-center bg-no-repeat opacity-60 grayscale-[50%]" />
          ...
          <div className="bg-black/60 backdrop-blur-md rounded-3xl border border-white/10 shadow-2xl p-6 md:p-10 text-center space-y-6">
            <div className="flex items-center justify-center gap-2">
              <span className="inline-flex items-center gap-1.5 text-[9px] font-extrabold text-emerald-500 bg-emerald-950/40 border border-emerald-900/40 px-3 py-1 rounded-full uppercase tracking-widest">
                <Activity className="w-3.5 h-3.5 text-emerald-500" /> Golf Mechanics
              </span>
              <span className="text-[10px] text-zinc-400 font-mono">CODE: GOLF-LAB-01</span>
            </div>

            <h1 className="text-4xl md:text-6xl font-black tracking-tight text-white leading-tight">
              ゴルフ<span className="text-emerald-500 font-extrabold relative inline-block">研究所</span>
            </h1>
              ...
              {roadmapSteps.map((st, idx) => (
                ...
                >
                  ...
                  <div className="mt-8 pt-3 border-t border-zinc-900 flex items-center justify-between text-[9px] font-mono text-zinc-500">
                    <span>GOLF-STEP-{st.slug.slice(0, 6)}</span>
      ...
      <section className="py-20 bg-black border-b border-zinc-900">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-4">
            {OBSTACLES.map((obs) => (
              <Link
                href={`/lab/golf/articles?obstacle=${obs.label}`}
                key={obs.label}
