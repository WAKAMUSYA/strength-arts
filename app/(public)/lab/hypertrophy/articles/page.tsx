...
function ArticlesContent() {
  ...
  const initialTab = initialTabStr === 'applied' ? 'applied' : initialTabStr === 'practical' ? 'practical' : 'basic'
  const initialObstacle = searchParams.get('obstacle') || null
  
  const [activeTab, setActiveTab] = useState<'basic' | 'applied' | 'practical'>(initialTab)
  ...
  useEffect(() => {
    const fetchStatuses = async () => {
      const ids = HYPERTROPHY_ARTICLES.map(art => `hypertrophy/${art.slug}`)
      try {
  ...
  const handleTabChange = (tab: 'basic' | 'applied' | 'practical') => {
    setActiveTab(tab);
  ...
  const filteredArticles = HYPERTROPHY_ARTICLES.filter(art => {
    ...
    <div className="max-w-6xl mx-auto px-6 mt-8">
        ...
        <div className="flex bg-zinc-950 p-1.5 rounded-2xl border border-zinc-900 shadow-inner w-full max-w-2xl">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-xs md:text-sm transition-all duration-300 cursor-pointer ${
              activeTab === 'basic'
                ? 'bg-gradient-to-r from-red-950/60 to-red-900/40 border border-red-800/40 text-white shadow-md'
                : 'text-zinc-550 hover:text-zinc-300'
            }`}
          >
            <BookOpen className={`w-4 h-4 ${activeTab === 'basic' ? 'text-red-400' : 'text-zinc-600'}`} />
            <span>基本理論コラム</span>
          </button>
          <button
          ...
          >
            <Layers className={`w-4 h-4 ${activeTab === 'applied' ? 'text-red-400' : 'text-zinc-600'}`} />
            <span>応用・探究コラム</span>
          </button>
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-xs md:text-sm transition-all duration-300 cursor-pointer ${
              activeTab === 'practical'
                ? 'bg-gradient-to-r from-red-950/30 to-red-900/20 border border-red-900/30 text-white shadow-md'
                : 'text-zinc-550 hover:text-zinc-300'
            }`}
          >
            <Dumbbell className={`w-4 h-4 ${activeTab === 'practical' ? 'text-red-400' : 'text-zinc-600'}`} />
            <span>実践プログラム</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        {filteredArticles.map((art) => {
          const articleId = `hypertrophy/${art.slug}`;
          ...
          >
            ...
            <div className="space-y-5 relative z-10">
              ...
              <div className="space-y-1">
                <h3 className="text-lg md:text-xl font-black text-white group-hover:text-red-400 transition-colors leading-tight">
                  {art.title}
                </h3>
              </div>
            ...
            <div className="mt-8 pt-4 border-t border-zinc-900 flex flex-wrap items-center justify-between gap-3 text-[11px] font-mono text-zinc-500 relative z-10">
              ...
              <span className="text-zinc-400 group-hover:text-red-400 group-hover:translate-x-1 transition-all flex items-center gap-1 font-bold">
                解説を開く <ChevronRight className="w-4 h-4 text-red-500" />
              </span>
            </div>
          </Link>
          )
...
export default function HypertrophyArticlesPage() {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
    ...
    <main className="min-h-screen bg-black text-white selection:bg-red-900 selection:text-white pb-32">
      ...
      <nav className="border-b border-zinc-900 bg-zinc-950/40 backdrop-blur-md sticky top-16 z-30 px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <Link 
            ...
            className="text-xs font-mono font-bold text-zinc-500 hover:text-red-400 transition-colors inline-flex items-center gap-1 cursor-pointer"
          >
            <ArrowLeft className="w-4 h-4" /> BACK TO LAB
          </Link>
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1 text-[8px] font-extrabold text-red-400 bg-red-950/40 border border-red-900/40 px-2.5 py-0.5 rounded-full uppercase tracking-wider">
              <Dumbbell className="w-3.5 h-3.5 text-red-500" /> LECTURE INDEX
            </span>
            <span className="text-[10px] text-zinc-550 font-mono hidden sm:inline">SA-HYP-LECTURES</span>
          </div>
        </div>
      </nav>
      ...
      <header className="max-w-6xl mx-auto px-6 pt-16 pb-8 text-center space-y-4">
        <h1 className="text-3xl md:text-5xl font-black tracking-tight text-white">
          筋肥大研究所<br className="sm:hidden" /> コラムアーカイブ
        </h1>
      </header>
