...
for (const cat of dirs) {
  if (cat === 'benchpress') continue; // Already done
  ...
  if (fs.existsSync(slugPagePath)) {
    ...
    if (!slugCode.includes('getSAMemberStatus')) {
      ...
      // Some might have `import { useParams, notFound }` -> if so, it becomes `import { notFound }`
      slugCode = slugCode.replace(/useParams,\s*/, '');
      slugCode = slugCode.replace(/,\s*useParams/, '');
...
`;
      ...
      const notFoundMatch = slugCode.match(/if \(!article\) \{[\s\S]*?\}/);
      if (notFoundMatch) {
        slugCode = slugCode.replace(notFoundMatch[0], `${notFoundMatch[0]}\n${lockLogic}`);
      }
